		...
		<div class="col-md-12">
			<div class="portlet light ">
				<div class="portlet-body">
					<form role="form" class="form-horizontal" action="<?=base_url('member_side/perbarui_laporan_sarling');?>" method="post" enctype='multipart/form-data'>
                         <input type="hidden" name="id_sarling" value="<?= md5($data_utama->id_sarling); ?>">
                         <input type="hidden" name="id_laporan_sarling" value="<?= $this->uri->segment(3); ?>">
                        <div class="form-body">
                                                        ...
                                                        foreach ($get_indikator as $key => $value) {
                                                            ...
                                                            for ($l=0; $l < count($list_indikator); $l++) {
                                                                if($list_indikator[$l]==$value->id_indikator){
                                                                    $check_value = $this->Main_model->getSelectedData('detail_laporan_sarling_aspek_fisik a', 'a.*',array('md5(a.id_laporan_sarling)'=>$this->uri->segment(3),'a.indikator_progres_fisik'=>$value->id_indikator))->row();
                                                                    if($check_value==NULL){
                                                ...
                                                <td>
                                                    <div class="md-checkbox-list">
                                                        ...
                                                        foreach ($get_indikator as $key => $value) {
                                                            ...
                                                            for ($l=0; $l < count($list_indikator); $l++) {
                                                                if($list_indikator[$l]==$value->id_indikator){
                                                                    $get_keterangan = $this->Main_model->getSelectedData('detail_laporan_sarling_aspek_fisik a', 'a.*',array('a.indikator_progres_fisik'=>$value->id_indikator,'b.id_sarling'=>$data_utama->id_sarling,'b.deleted="0"'),'b.created_at DESC','1','','',array(
                                                                        'table' => 'laporan_sarling b',
                                                                        'on' => 'a.id_laporan_sarling=b.id_laporan_sarling',
                                                                        'pos' => 'LEFT'
                                                                    ))->row();
                                                                    if(md5($get_keterangan->id_laporan_sarling)==$this->uri->segment(3)){
							...
							foreach ($indikator as $key => $i) {
                                $detail_laporan_sarling_aspek_keuangan = $this->Main_model->getSelectedData('detail_laporan_sarling_aspek_keuangan a', 'a.*',array('md5(a.id_laporan_sarling)'=>$this->uri->segment(3),'a.id_master_indikator'=>$i->id_master_indikator))->row();
                            ?>
							<div class="form-group form-md-line-input has-danger">
								<label class="col-md-2 control-label" for="form_control_1"><?= $i->master_indikator; ?></label>
								<div class="col-md-10">
									<div class="input-icon">
										<input type="number" class="form-control" name="progres_keuangan_<?= $i->id_master_indikator; ?>" placeholder="Type something" value='<?= $detail_laporan_sarling_aspek_keuangan->progres_keuangan; ?>'>
										<div class="form-control-focus"> </div>
										<span class="help-block">Some help goes here...</span>
										<i class="fa fa-list"></i>
									</div>
