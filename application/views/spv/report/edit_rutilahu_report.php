...
<ul class="page-breadcrumb breadcrumb">
	...
	<li>
		<span><a href='<?= site_url('/spv/laporan_rutilahu'); ?>'>Data Rutilahu (Rumah Tidak Layak Huni)</a></span>
		<i class="fa fa-circle"></i>
	</li>
	<li>
		<span>Ubah Data</span>
		...
		<div class="col-md-12">
			<div class="portlet light ">
				<div class="portlet-body">
					<form role="form" class="form-horizontal" action="<?=base_url('spv/perbarui_laporan_rutilahu');?>" method="post" enctype='multipart/form-data'>
                         <input type="hidden" name="id_rutilahu" value="<?= md5($data_utama->id_rutilahu); ?>">
                         <input type="hidden" name="id_laporan_rutilahu" value="<?= $this->uri->segment(3); ?>">
                        <div class="form-body">
							<div class="form-group form-md-line-input has-danger">
								<label class="col-md-2 control-label" for="form_control_1">Nama Kelompok <span class="required"> * </span></label>
								<div class="col-md-10">
                                    <div class="input-icon">
                                        <input type="text" class="form-control" value="<?= $data_utama->nama_kelompok.' - '.$data_utama->nm_kabupaten; ?>" readonly>
                                                        ...
                                                        foreach ($get_indikator as $key => $value) {
                                                            ...
                                                            for ($l=0; $l < count($list_indikator); $l++) {
                                                                if($list_indikator[$l]==$value->id_indikator){
                                                                    $check_value = $this->Main_model->getSelectedData('detail_laporan_rutilahu_aspek_fisik a', 'a.*',array('md5(a.id_laporan_rutilahu)'=>$this->uri->segment(3),'a.indikator_progres_fisik'=>$value->id_indikator))->row();
                                                                    if($check_value==NULL){
                                                ...
                                                <td>
                                                    <div class="md-checkbox-list">
                                                        ...
                                                        foreach ($get_indikator as $key => $value) {
                                                            ...
                                                            for ($l=0; $l < count($list_indikator); $l++) {
                                                                if($list_indikator[$l]==$value->id_indikator){
                                                                    $get_keterangan = $this->Main_model->getSelectedData('detail_laporan_rutilahu_aspek_fisik a', 'a.*',array('a.indikator_progres_fisik'=>$value->id_indikator,'b.id_rutilahu'=>$data_utama->id_rutilahu,'b.deleted="0"'),'b.created_at DESC','1','','',array(
                                                                        'table' => 'laporan_rutilahu b',
                                                                        'on' => 'a.id_laporan_rutilahu=b.id_laporan_rutilahu',
                                                                        'pos' => 'LEFT'
                                                                    ))->row();
                                                                    if(md5($get_keterangan->id_laporan_rutilahu)==$this->uri->segment(3)){
							...
							foreach ($indikator as $key => $i) {
                                $detail_laporan_rutilahu_aspek_keuangan = $this->Main_model->getSelectedData('detail_laporan_rutilahu_aspek_keuangan a', 'a.*',array('md5(a.id_laporan_rutilahu)'=>$this->uri->segment(3),'a.id_master_indikator'=>$i->id_master_indikator))->row();
                            ?>
							<div class="form-group form-md-line-input has-danger">
								<label class="col-md-2 control-label" for="form_control_1"><?= $i->master_indikator; ?></label>
								<div class="col-md-10">
									<div class="input-icon">
										<input type="number" class="form-control" name="progres_keuangan_<?= $i->id_master_indikator; ?>" placeholder="Type something" value='<?= $detail_laporan_rutilahu_aspek_keuangan->progres_keuangan; ?>'>
										<div class="form-control-focus"> </div>
										<span class="help-block">Some help goes here...</span>
										<i class="fa fa-list"></i>
									</div>
