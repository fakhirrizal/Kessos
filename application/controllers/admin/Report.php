...
class Report extends CI_Controller {
	...
	public function save_kube_report(){
		...
		$get_data_kube = $this->Main_model->getSelectedData('kube a', 'a.*', array('a.id_kube'=>$this->input->post('id_kube')))->row();
	...
	// public function json_laporan_kube(){
	// 	$get_data = $this->Main_model->getSelectedData('laporan_kube a', 'a.*,b.fullname',array('md5(a.id_kube)'=>$this->input->get('id'),'a.deleted'=>'0'),'','','','',array(
	// 		'table' => 'user_profile b',
	// 		'on' => 'a.user_id=b.user_id',
	// 		'pos' => 'LEFT',
	// 	))->result();
	...
	public function detail_rutilahu_report()
	{
		...
		$data['data_utama'] = $this->Main_model->getSelectedData('rutilahu a', 'a.*,e.jenis_usaha,f.nm_provinsi,b.nm_kabupaten,c.nm_kecamatan,d.nm_desa', array('md5(a.id_rutilahu)'=>$this->uri->segment(3),'a.deleted'=>'0'),'','','','',array(
			array(
				'table' => 'provinsi f',
				'on' => 'a.id_provinsi=f.id_provinsi',
				'pos' => 'left',
